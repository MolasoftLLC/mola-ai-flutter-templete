...
SAKEPEDIAの保存済み日本酒を根拠に店舗と日本酒を匿名で紐づけ、次の双方向検索をFlutterアプリへ実装する。

- 日本酒名から、その日本酒が登録されている店舗を地図で探す。
- 地図上の店舗から、そこに登録されている日本酒を探す。
- 投稿公開、店舗×日本酒の登録、マップ写真公開を分離する。
...
`Sake.place`は既存データ互換のため削除しない。新規データでは`drinkingPlace`を優先表示し、古いデータは`place`へフォールバックする。新しい店舗登録はGoogle候補だけを受け付ける。
...
- 手入力候補は設けず、Google候補の選択だけを登録として扱う。
...
- 店舗選択時点で店舗×日本酒情報を匿名でマップへ登録し、初回5ptを付与する。
- 写真公開だけを任意設定とし、初回公開時に追加10ptを付与する。
- 写真を公開しない場合も店舗×日本酒の紐付けは有効とする。
- 投稿の`isPublic`とマップ写真公開は互いに依存させない。
...
- 画面上部に日本酒名検索欄を置く。
- 日本酒名の候補は店舗登録が存在する日本酒だけを表示し、各候補に登録店舗数を表示する。
- Google Map上に店舗単位のピンを表示する。
- 店舗ピンは`latestImageUrl`を円形に切り抜いた画像ピンとし、未設定・取得失敗時は標準ピンへ戻す。
- 同じ店舗の複数記録を複数ピンにせず、1つの店舗ピンへ集約する。
- ピンには日本酒種類数または飲酒記録数を表示できる構造にする。
- ピンタップでボトムシートを開き、「この店舗に登録されている日本酒」を表示する。
- ボトムシートの日本酒タップで`sake_master`由来の詳細画面を開き、`new_home`に合わせた配色・カード表現にする。
- 詳細のメイン画像には`primaryImageUrl`を使用し、未設定・取得失敗時は`assets/images/sake_placeholder.png`を表示する。
- 日本酒選択後は、その日本酒が登録されている店舗だけを地図へ表示する。
...
`sakeId`は任意。マップ登録は日本酒マスターIDが確定している保存酒だけを対象にする。
...
各要素は`sakeId`、`name`、`brewery`、`type`、`recordCount`、`primaryImageUrl`を返す。`primaryImageUrl`は`sake_master`のメイン画像とし、タップ後は既存の`GET /api/sakes/{sakeId}/overview`から詳細を取得する。マスターIDや画像がない場合も画面を開き、一覧情報とNoImageを表示する。

表示文言は「この店舗に登録されている日本酒」とし、現在の提供・在庫を保証しない旨を必要に応じて示す。

### 7. 公開範囲

- 店舗×日本酒の紐付けは投稿の`isPublic`に依存させない。
- 写真は`mapPhotoPublic == true`の登録からだけ表示する。
...
- 写真非公開の画像が公開マップへ混入しないことをAPIモックで確認する。
